...
(function ($) {
    /* DatePicker  */
    $(
        "#datepicker,#datepicker2,#datepicker3,#datepicker4,#datepicker5,#datepicker6,#datepicker7"
    ).datepicker({
        dateFormat: "d MM yy",
        duration: "medium",
        changeMonth: true,
    ...
    /* Select */
    $(
        "#countryOption,#cityOption,#skillsOption,#exampleFormControlSelect1,#select-countryOption"
    ).select2({
    ...
    $(".kb__select-wrapper select,.tagSelect-rtl select").select2({
        ...
        dropdownParent: $(
            ".kb__select-wrapper .select2,.tagSelect-rtl .select2"
        ),
    });
    ...
    if ($(".modal--selected").length) {
        var fileM = new bootstrap.Modal(
            document.querySelector(".modal--selected")
        );
        fileM.show();
    }
    ...
    $("#mail-message, #mail-reply-message").trumbowyg({
        svgPath:
            typeof env !== "undefined" && env.editorIconUrl
                ? env.editorIconUrl
                : "img/ui/icons.svg",
        btns: [
            ["viewHTML"],
    ...
    $(".job-value2").text($(".price-slider").slider("values", 1) + " miles");
    ...
    if (document.getElementsByClassName("new-member__2").length) {
        var myMember = new bootstrap.Modal(
            document.getElementById("new-member2")
        );
        myMember.show();
    }
    ...
    $.fn.dateRangePicker = function (options) {
        ...
        options.beforeShow = function (input, inst) {
            var dateFormat = myDateRangeTarget.datepicker(
                "option",
                "dateFormat"
            );
            ...
            if (!inst.rangeStart) {
                ...
                if (textStart !== dateText) {
                    ...
                    if (options.useHiddenAltFields) {
                        ...
                        var myFromField =
                            myDateRangeTarget.attr("data-from-field");
...
//   });
    $(document).ready(function () {
      ...
      $(".is_display").click(function () {
            ...
            $.ajax({
                type: 'GET',
                url: url+'/admin/image_display/'+row_id+'/'+ current_val,
                beforeSend: function() {
                },
